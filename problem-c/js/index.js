...
huskyGames2016.filter(huskiesLost).forEach(i =>
  console.log(extractOpponent(i) + " at", i.home + ", " + i.opponent_score + " to " + i.home_score)
);
...
console.log(huskyGames2016.filter(function(game) {return game.fumbles > 0}).length);
...
function mostYardsPassing(game1, game2) {
  if (game1.passing_yards == null || game1.passing_yards < game2.passing_yards) {
    return game2;
  } else {
    return game1;
  }
...
function makeCombinedFilter(fun1, fun2) {
  return function combinedFilter(game) {
    return fun1(game) && fun2(game);
  };
}
